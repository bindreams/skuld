...
Then bump every publishable member's manifest to `X.Y.Z+1` (`publishable-members.sh` above lists them; today that is `Cargo.toml`, `macros/Cargo.toml` and `cargo-skuld/Cargo.toml`), fix the root cause, and re-run both workflows with the new version. Because the bump is lockstep, `cargo-skuld` then has no `X.Y.Z` at all — a gap in its version line is the accepted cost of a shared workspace version, not a problem to work around.
...
Pin `--target` explicitly rather than trusting the draft's current target field. The automated flip does the same, and for the same reason: an edit to the draft since the run resolved the SHA would otherwise tag a commit that was never actually published. That race is real even inside the run itself — the gap between resolving the SHA and flipping spans the verification build and the uploads, tens of minutes, not a negligible few seconds — and only widens for a manual recovery happening well after the run. Read `SHA` from the failed run's own **"Verify draft release exists and resolve commit SHA"** step, which prints `Resolved vX.Y.Z -> <sha>` to both the log and the job summary, not from re-querying the live draft:
